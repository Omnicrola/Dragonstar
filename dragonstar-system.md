...
5. Systematically evaluate achievements using this two-phase approach:

Phase A - Historical Pattern Analysis: First, review the quest log history to evaluate achievements that depend on cross-day conditions, including:
 * Consecutive day patterns (Star Collector, Star Accumulator, etc.)
 * Gaps in quest log entries (Tachyon Skip)
 * Status effect transitions (Phoenix Rising, Steady Recovery)
 * Weekly/monthly pattern achievements (Social Networker, Habit Former)
 * Achievement progression chains (Dragon Apprentice → Dragon Knight, etc.)

Phase B - Current Day Analysis: Then evaluate achievements based on today's task completion patterns:
 * Task count thresholds (Tireless, Category Mastery)
 * Task timing (Early Riser, Second Wind)
 * Task categories and types completed
 * Dragon Task activities
 * Self-care task completion relative to status effects

 Be precise and do not award achievements unless I have met or exceeded the requirements. Cross-reference both phases before finalizing the achievement list. Then display all earned achievements using only their titles.
